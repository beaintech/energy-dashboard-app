...
你：
筛选与搜索在 FilterBar.vue 中产生，向上触发 update:filter、update:search 事件；在 DashboardPage.vue 中接收后，同步到路由 query。为了避免频繁渲染，我对关键字搜索做了 300ms 防抖；对于德语变音字，我做了简单的"规范化"处理（把 ä/ö/ü/ß 统一到 ae/oe/ue/ss 的可选匹配），同时把大小写忽略，确保"Düsseldorf"与"Dusseldorf"能匹配到同一条。
*/
...
// 本地状态，初始化从 props
const statusLocal = ref(props.status || 'all');
...
watch(() => props.status, (v) => {
  if (v !== undefined && v !== statusLocal.value) {
    statusLocal.value = v;
  }
}, { immediate: true });

watch(() => props.search, (v) => {
  if (v !== undefined && v !== searchLocal.value) {
    searchLocal.value = v;
  }
}, { immediate: true });

// 监听下拉框变化 → 直接同步到父组件
// 跳过初始化时的触发，只在用户交互时才发送
watch(statusLocal, (v) => {
  if (!isInitialized) {
    isInitialized = true;
    return;
...
watch(searchLocal, (v) => {
  ...
  debounceTimer = window.setTimeout(() => emit('update:search', v), 300);
});
</script>
